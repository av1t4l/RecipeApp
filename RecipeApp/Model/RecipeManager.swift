...
//        let Nut2 = [Nutrient(name:"Energy", amount:30, unitName:Unit.g), carbs , Nutrient(name:"Protien", amount:4, unitName:Unit.g), fats, Nutrient(name:"Fibre", amount:20, unitName:Unit.g), Nutrient(name:"Sodium", amount:300, unitName:Unit.mg) ]
        ...
        for i in Ings {
            let tempNutr = req.getNutrients(ingrStr: i.ingString())
            let defRecipe1 = Recipe(title:"Pancakes", mealTypes: [MealType.breakfast], dietaryReqs:[DietaryReq.lf], time:Time(time:30, unit:"m"), diff: Diff.Easy, serves:2, ingredients: Ings, method:Method, image: "pancakes", nutrients: tempNutr )
                recipes.append(defRecipe1)
        }
        //WANT TO USE FULLY REIOE ENDPOINT INSTEAD
...
//        recipes.append(defRecipe3)
    ...
    mutating func addRecipe(recipe: Recipe) {
        ...
        let req = REST_Request()
        req.getNutrients(ingrStr: "100ml milk")
    }
